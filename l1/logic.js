// Save our API endpoint as queryURL
...
function createFeatures(earthquakeData, platesData){
    // Attach a popup to each feature showing place and time of earthquakes
    ...
    // Generate GeoJSON layer with features array from earthquakeData object
    ...
    // Create variable for earthquakes to store latlng, popup features, and circle attributes
    let earthquakes = L.geoJSON(earthquakeData, {
        onEachFeature: onEachFeature,
        pointToLayer: createCircleMarker
    });

    // Pass earthquakes layer to createMap function to begin map creation and add features
    createMap(earthquakes);
}

// Circle color palette based on mag (feature) data marker: data markers should indicate earthquake magnitude by size and depth by color. Larger magnitude earthquakes should be bigger, and deeper earthquakes should be darker.
...
// Generate map legend for context
...
legend.onAdd = function() {
    ...
    // Label and color the legend for each magnitude item
    // Add to labels array as list item
    for (var i = 0; i < grades.length; i++) {
        labels.push('<ul style="background-color:' + chooseColor(grades[i] + 1) + '"> <span>' + grades[i] + (grades[i + 1] ? '&ndash;' + grades[i + 1] + '' : '+') + '</span></ul>');
    }

    // Insert label list items in div under <ul> tag
